...
async def account_summary():
    ...
    broker = load_state().get("brokers", {}).get("zerodha", {})
    result = {"available": 0, "day_pnl": 0, "total_pnl": 0, "positions_pnl": 0, "holdings_pnl": 0, "holdings_day_pnl": 0, "source": "none"}
    if broker.get("api_key") and broker.get("access_token"):
        try:
            ...
            async with httpx.AsyncClient(timeout=12.0, headers=headers) as client:
                ...
                holdings_day_pnl = sum(float(h.get("day_change", 0)) for h in holdings)
            result = {"available": round(available, 2), "positions_pnl": round(day_pnl, 2), "holdings_pnl": round(holdings_pnl, 2), "holdings_day_pnl": round(holdings_day_pnl, 2), "day_pnl": round(day_pnl, 2), "total_pnl": round(day_pnl + holdings_pnl, 2), "source": "zerodha"}
        except Exception as e:
            result["error"] = str(e)[:200]
    return result
